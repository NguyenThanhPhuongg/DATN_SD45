...
document.addEventListener('DOMContentLoaded', () => {
    ...
    input.addEventListener('keypress', (event) => {
        if (event.key === 'Enter') {
            event.preventDefault();
            if (input.value.trim()) {
                form.submit(); // Submit form nếu từ khóa hợp lệ
            } else {
                alert('Vui lòng nhập từ khóa để tìm kiếm!');
            }
        }
...
$(document).ready(function () {
    ...
    if (keyword) {
                    ...
                    data.forEach(product => {
                        const productHtml = `
                                <div class="col-md-3 mb-3">
                                    <div class="card">
                                        <img src="${product.anh || '/images/default.png'}" class="card-img-top" alt="${product.ten}">
                                        <div class="card-body">
                                            <h5 class="card-title">${product.ten}</h5>
                                            <p class="card-text">Giá: ${product.gia.toLocaleString()} VND</p>
        ...
        $('#searchResults').html('<p class="text-danger">Vui lòng nhập từ khóa tìm kiếm.</p>');
    }
});
